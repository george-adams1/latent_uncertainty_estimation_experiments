...
Status: implemented; pilot and frozen confirmatory runs complete. The prompts
are frozen by git commit `79554bc`; any later prompt edit requires a new version
and a complete rerun. Confirmatory results are in
`EC_CLUSTERING_FULL_RESULTS.md`.
...
For the actual frozen run, prefer the one-step launcher below. It starts the
TP8 Llama-70B clusterer, waits for readiness, runs Qwen3-8B and then Llama-70B
sequentially, and stops only the server process when finished:

```bash
srun --jobid=JOB_ID --overlap --nodes=1 --ntasks=1 \
  --cpus-per-task=16 --mem=32G \
  scripts/run-full-ec-clustering.sh
```

The launcher uses prompt-freeze commit `79554bc`, excludes the final pilot
manifests, and writes `ec_clustering_full_{qwen3_8b,llama3_70b}.jsonl` plus
their summary JSON files under `scan_results/`. It refuses to overwrite an
existing result. `RESUME=1` is only for resuming an interrupted run with the
same frozen inputs and settings.
...
- `scripts/run-full-ec-clustering.sh`: single-step TP8 server and sequential
  full-run launcher
