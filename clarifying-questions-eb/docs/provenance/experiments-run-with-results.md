# E-B: what was actually run, with results
...
version deviates from the spec, that's called out explicitly with the
reason. This is `experiments-run.md` plus the actual numbers from the two
final runs.
...
### Matching and leak audit

| | Set A matched / target | Set B matched / target | Self-ask leak rate |
|---|---|---|---|
| Qwen3-8B | 20/20 | 20/20 | 0/20 (0.0%) |
| Qwen2.5-72B-Instruct | 20/20 | 19/20 | 0/20 (0.0%) |

Zero leaks in both final runs — every self-ask reply was checked against the
intended answer's alias list post-hoc, and none of the 40 replies (20 per
run) contained it.

### Accuracy by condition

| | Set A answer-now | Set A oracle-clarify (ceiling) | Set A self-ask | Set B answer-now | Set B self-ask |
|---|---|---|---|---|---|
| Qwen3-8B | 0% | 15% | 0% | 20% | 25% |
| Qwen2.5-72B-Instruct | 5% | 25% | 25% | 26.3% | 26.3% |

### The three predictions

**Prediction 1 — gain from asking should be large on Set A, near zero on
Set B.**

| | Set A oracle gain (ceiling) | Set A self-ask gain (realized) | Set B self-ask gain |
|---|---|---|---|
| Qwen3-8B | +15pp | **+0pp** | +5pp |
| Qwen2.5-72B-Instruct | +20pp | **+20pp** | +0pp |

Qwen3-8B shows the ceiling value is real (oracle-clarify gains 15 points
over answering blind) but self-asking captured none of it — the model could
not convert its own clarifying question and the simulated reply into a
better answer. Its Set B gain (+5pp) is small but, awkwardly, larger than
its own Set A realized gain, which is not the clean pattern predicted.

Qwen2.5-72B-Instruct shows the predicted pattern cleanly: self-ask gain
exactly equals the oracle ceiling (both +20pp) — full value captured — and
Set B gain is exactly 0. This is the clearest confirmation of prediction 1
across both runs.

**Prediction 2 — pre-ask confidence should not predict the gain.**

| | Confidence-vs-gain correlation |
|---|---|
| Qwen3-8B | -0.039 |
| Qwen2.5-72B-Instruct | -0.066 |

Confirmed in both runs — both correlations are small and negative,
consistent with "no relationship," as predicted.

**Prediction 3 — the typed diagnostic should predict the gain.**

| | Entropy-vs-gain correlation | Second-largest-cluster-vs-gain correlation |
|---|---|---|
| Qwen3-8B | +0.154 | +0.090 |
| Qwen2.5-72B-Instruct | +0.166 | +0.136 |

Weakly confirmed in both runs — all four correlations are positive (the
predicted direction), but small in magnitude. With n=39-40 pooled items per
run, none of these should be treated as a statistically solid result; they
are directionally consistent with the prediction, not strong evidence for
it on their own.

Set A's hedge rate (answer naming both readings) was 0.0% in both runs —
neither model hedged on the immediate-answer condition in this sample;
Qwen3-8B was simply wrong, Qwen2.5-72B-Instruct was mostly wrong with a
20% straight hit rate.

### Reading the two runs together

The two models produced qualitatively different failure/success patterns
rather than just different accuracy levels. Qwen3-8B demonstrates that
ceiling value from disambiguation can exist (prediction 1's premise) while
completely failing to execute on it through self-asking — a competence gap
consistent with `experiment-ask-protocol.md`'s framing of the
self-ask/oracle-gain ratio as "a competence measure: how much of the
available value the model captures" (0% for Qwen3-8B). Qwen2.5-72B-Instruct
captures 100% of a larger ceiling, giving the cleanest full confirmation of
prediction 1 obtained. Both agree on predictions 2 and 3's direction. Given
n≈20 per set per run, none of this should be read as a statistically
powered result — it's a working demonstration that the harness produces
the qualitative pattern the theory predicts, on two real models, not a
...
- Qwen3-8B's Set B gain (+5pp) being larger than its own Set A realized
  gain (+0pp) is a genuine wrinkle in the results, not smoothed over above —
  worth a second look before citing this run as clean support for
  prediction 1.
